...
import { User, Mail, Users, Lock, Eye, EyeOff, Save, Shield, Edit2, Phone, MapPin, Camera, Trash2 } from 'lucide-react';
...
const SettingsDemo = () => {
    ...
    const handlePhotoUpload = async (e) => {
        try {
            ...
            console.error('Detailed error in handlePhotoUpload:', error);
            setMessage({ type: 'error', text: `Error: ${error.message || 'Failed to upload profile photo'}` });
        } finally {
    ...
    const handleDeletePhoto = async () => {
        try {
            ...
            setEditedProfile({ ...editedProfile, avatar_url: null });
            ...
            if (user) {
                ...
                if (profile) {
                    ...
                    if (managedProjects && managedProjects.length > 0) {
                        const mProjects = managedProjects
                            .filter(p => !allProjects.some(ap => ap.projectName === p.name)) // Deduplicate
                            .map(p => ({
                                projectName: p.name,
                                role: 'Manager'
                            }));
                        allProjects.push(...mProjects);
                    }



                    // Ensure Current Project from Context is included
                    if (currentProject) {
                        const contextProjExists = allProjects.some(ap => ap.projectName === currentProject.name);
                        if (!contextProjExists) {
                            allProjects.unshift({
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                    ...
                    <div>
                        ...
                        <div style={{
                            ...
                            {userProfile?.role === 'employee' ? 'Student' : (userProfile?.role || 'N/A')}
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div style={{
                            ...
                            {projectRoles.length > 0 ? projectRoles[0].projectName : (departmentName || 'No project assigned')}
                        </div>
                    </div>
                </div>
            </div>
