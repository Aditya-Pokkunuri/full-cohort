            ...
            try {
                ...
                if (teamsData) {
                    teamsData.forEach(team => {
                        ...
                        const projectMembers = teamMembersData?.filter(tm => tm.project_id === team.id) || [];
                        const projectMemberIds = projectMembers.map(tm => tm.user_id);

                        // Find Manager
                        const managerMember = projectMembers.find(tm => tm.role?.toLowerCase() === 'manager');
                        let managerName = 'N/A';
                        if (managerMember) {
                            const p = employeesData.find(e => e.id === managerMember.user_id);
                            if (p) managerName = p.full_name;
                        }
