...
const ProjectDocuments = ({ userRole, addToast: parentAddToast = null }) => {
    ...
    const [newDoc, setNewDoc] = useState({ title: '', content: '', doc_type: 'requirements', project_id: null });
    const [file, setFile] = useState(null);
    const [uploading, setUploading] = useState(false);
    const [viewingDoc, setViewingDoc] = useState(null);
    const [projects, setProjects] = useState([]);

    const canEdit = userRole === 'executive' || projectRole === 'manager' || projectRole === 'team_lead';
    ...
    const fetchDocuments = async () => {
        ...
        try {
            ...
            const formatRole = (role) => {
                ...
                if (role.toLowerCase() === 'employee') return 'Student';
                return role.split('_').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
            };

            // Attach project names and uploader info, and generate signed URLs
            const enhancedData = await Promise.all((data || []).map(async doc => {
                const uploader = profileMap[doc.created_by];

                // Generate signed URL if file exists
                let signedUrl = null;
                const filePath = doc.file_url; // Store original path for extension detection
                if (doc.file_url) {
                    try {
                ...
                return {
                    ...doc,
                    file_path: filePath, // Keep original path for extension detection
                    file_url: signedUrl, // Replace with signed URL for access
                    ...
                    uploader_role: formatRole(uploader?.role)
                };
            }));
    ...
    const handleAddDocument = async () => {
        ...
        console.log('DEBUG: Project ID being sent:', currentProject?.id || newDoc.project_id);
        ...
        // For executives, require project selection
        const projectId = userRole === 'executive' ? newDoc.project_id : currentProject?.id;
        if (!projectId) {
        ...
        try {
            ...
            setNewDoc({ title: '', content: '', doc_type: 'requirements', project_id: null });
            setFile(null);
            fetchDocuments();
        } catch (err) {
            ...
            {showAddModal && (
                <div style={{
                    ...
                    <div style={{
                        ...
                        {userRole === 'executive' && (
                            <div style={{ marginBottom: '16px' }}>
                                <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 600, marginBottom: '6px', color: '#374151' }}>Project *</label>
                                <select
                                    value={newDoc.project_id || ''}
                                    onChange={(e) => setNewDoc({ ...newDoc, project_id: parseInt(e.target.value) })}
                                    style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #e5e7eb', fontSize: '0.9rem' }}
                                >
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        <div style={{ flex: 1, overflow: 'hidden', backgroundColor: '#f8fafc', display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '0' }}>
                            {(() => {
                                // Extract extension from original file path, not the signed URL
                                const ext = viewingDoc.file_path?.split('.').pop()?.toLowerCase();
