...
const ModulePage = ({ title, type }) => {
    ...
    useEffect(() => {
        ...
        const fetchTeamMembers = async () => {
            try {
                ...
                memberIds.forEach(m => projectRoleMap[m.user_id] = m.role);
                ...
                // 6. Fetch tasks for performance calculation
                ...
                const mappedMembers = profiles.map(member => {
                    ...
                    if (activeSet.has(member.id)) {
                        ...
                        status = orgName === 'Cohort' ? 'Away' : 'On Leave';
                    }

                    const completedTasks = tasksMap[member.id] || 0;
                    const totalTasks = totalTasksMap[member.id] || 0;
                    ...
                    return {
                        ...
                        role: projectRoleMap[member.id] || member.role || 'N/A',
                        dept: teamName,
    ...
    const handleDeleteLeave = async (leaveRequest) => {
        ...
        try {
            ...
            if (!isLossOfPay && leaveRequest.status === 'Pending') {
                // Skiping profile update as columns are missing in Cohort schema
            ...
            try {
                ...
                if (recipientIds.size > 0) {
                    const notificationRecords = Array.from(recipientIds).map(receiverId => ({
                        ...
                        org_id: orgId,
                        sender_name: userName || 'Student',
                        message: `${userName || 'Student'} has applied for ${leaveFormData.leaveType}`,
                        type: 'leave_request',
                        is_read: false,
                        created_at: new Date().toISOString()
                    }));
    ...
    if (type === 'project-documents') return <ProjectDocuments userRole={userRole} addToast={addToast} />;
                ...
                },
                {
                    ...
                    render: (row) => (
                        <span style={{ textTransform: 'capitalize' }}>
                            {row.role === 'employee' ? 'Student' : row.role}
                        </span>
                    )
                },
