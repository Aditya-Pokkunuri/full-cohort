...
const MyTasksPage = () => {
    ...
    const [taskForExtension, setTaskForExtension] = useState(null);
    ...
    const submitExtensionRequest = async () => {
        if (!extensionReason.trim()) {
            addToast?.('Please provide a reason for the extension', 'error');
            return;
        }
        if (!extensionHours || Number(extensionHours) <= 0) {
            addToast?.('Please enter valid additional hours', 'error');
            return;
        }
        ...
        try {
            ...
            const { error } = await supabase.from('extension_requests').insert({
                task_id: taskForExtension.id,
                requested_by: user.id,
                reason: extensionReason.trim(),
                additional_hours: parseFloat(extensionHours),
                org_id: profile?.org_id
            });

            if (error) throw error;

            addToast?.('Extension request submitted! Awaiting manager approval.', 'success');
            setShowExtensionModal(false);
            setTaskForExtension(null);
        } catch (error) {
            console.error('Error submitting extension request:', error);
            addToast?.('Failed to submit extension request: ' + error.message, 'error');
        } finally {
                                ...
                                return (
                                    <tr key={task.id} style={{
                                        ...
                                        <td style={{ padding: '16px' }}>
                                            <div>
                                                <div style={{ fontWeight: 600, color: isOverdue ? '#991b1b' : '#1e293b', fontSize: '0.95rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                    {task.title}
                                                    {isLocked && (
                                                        <span style={{ backgroundColor: '#fecaca', color: '#991b1b', fontSize: '0.7rem', padding: '2px 6px', borderRadius: '4px', fontWeight: 600 }}>
                                                            🔒 LOCKED
                                                        </span>
                                                    )}
                                                    {isOverdue && !isLocked && (
                                                        <span style={{ backgroundColor: '#fef3c7', color: '#92400e', fontSize: '0.7rem', padding: '2px 6px', borderRadius: '4px', fontWeight: 600 }}>
                                                            ⏰ OVERDUE
                                                        </span>
                                                    )}
                                                </div>
                                        ...
                                        <td style={{ padding: '16px', textAlign: 'center' }}>
                                            {task.status === 'completed' ? (
                                                ...
                                                <div style={{ display: 'flex', gap: '6px', justifyContent: 'center', whiteSpace: 'nowrap' }}>
                                                    ...
                                                    {(task.sub_state === 'in_progress' || task.sub_state === 'pending_validation' || task.sub_state === 'rejected') && (
                                                        <button
                                                            ...
                                                            style={{
                                                                padding: '6px 10px',
                                                                borderRadius: '6px',
                                                                backgroundColor: task.sub_state === 'pending_validation' ? '#f59e0b' : (task.sub_state === 'rejected' ? '#dc2626' : '#8b5cf6'),
                                                                color: 'white',
                                                                border: 'none',
                                                                fontWeight: 500,
                                                                cursor: 'pointer',
                                                            ...
                                                            onMouseEnter={(e) => e.currentTarget.style.backgroundColor = task.sub_state === 'pending_validation' ? '#d97706' : (task.sub_state === 'rejected' ? '#b91c1c' : '#7c3aed')}
                                                            onMouseLeave={(e) => e.currentTarget.style.backgroundColor = task.sub_state === 'pending_validation' ? '#f59e0b' : (task.sub_state === 'rejected' ? '#dc2626' : '#8b5cf6')}
                                                        >
                                                            <Upload size={12} />
                                                            {task.sub_state === 'pending_validation' ? 'Update Proof' : (task.sub_state === 'rejected' ? 'Re-submit' : 'Submit')}
                                                        </button>
                                                    )}
                                                    <button
            ...
            {showViewModal && taskForView && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1001, backdropFilter: 'blur(4px)' }}>
                    <div style={{ backgroundColor: 'var(--surface)', padding: '32px', borderRadius: '20px', width: '600px', maxWidth: '90%', maxHeight: '85vh', overflowY: 'auto', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)' }}>
                        ...
                        {(() => {
                            let parsedValidations = taskForView.phase_validations;
                            if (typeof taskForView.phase_validations === 'string') {
                                try {
                                    parsedValidations = JSON.parse(taskForView.phase_validations);
                                } catch (e) { }
                            }
                            const currentPhaseKey = taskForView.lifecycle_state || parsedValidations?.active_phases?.[0] || 'requirement_refiner';
                ...
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1001, backdropFilter: 'blur(4px)' }}>
                    <div style={{ backgroundColor: 'var(--surface)', padding: '32px', borderRadius: '20px', width: '500px', maxWidth: '90%', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px' }}>
                            ...
                            <div>
                                <h3 style={{ fontSize: '1.25rem', fontWeight: 700, color: 'var(--text-primary)' }}>Request Time Extension</h3>
                                <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>{taskForExtension.title}</p>
                            </div>
                        </div>
                        ...
                        <div style={{ marginBottom: '24px', padding: '16px', backgroundColor: '#fef2f2', borderRadius: '12px', border: '1px solid #fecaca' }}>
                            ...
                            <p style={{ fontSize: '0.85rem', color: '#7f1d1d', margin: 0 }}>
                                This task has exceeded its allocated time. You must request an extension from your project manager to continue working on it.
                            </p>
                        </div>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
                            <button
                                onClick={() => { setShowExtensionModal(false); setTaskForExtension(null); }}
                            ...
                            <button
                                ...
                                disabled={!extensionReason.trim() || !extensionHours || submittingExtension}
                                style={{
                                    padding: '12px 24px',
                                    borderRadius: '10px',
                                    background: (extensionReason.trim() && extensionHours) ? 'linear-gradient(135deg, #8b5cf6, #7c3aed)' : '#e5e7eb',
                                    color: (extensionReason.trim() && extensionHours) ? 'white' : '#9ca3af',
                                    border: 'none',
                                    fontWeight: 600,
                                    cursor: (extensionReason.trim() && extensionHours) ? 'pointer' : 'not-allowed',
                                    display: 'flex',
                                    alignItems: 'center',
                                    gap: '8px',
                                    boxShadow: (extensionReason.trim() && extensionHours) ? '0 4px 15px rgba(139, 92, 246, 0.3)' : 'none'
                                }}
                            >
                                <Send size={16} />
