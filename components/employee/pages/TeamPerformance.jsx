...
const TeamPerformance = () => {
    ...
    const fetchTeamPerformance = async () => {
        ...
        try {
            ...
            // Calculate daily progress (last 6 days)
            const last6Days = [];
            for (let i = 5; i >= 0; i--) {
                const d = new Date();
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            <style>{`
                ...
                .chart-bar-container-tp:hover .chart-tooltip-tp { 
                    opacity: 1; 
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        {dailyProgress.map((item, i) => {
                            const maxVal = Math.max(...dailyProgress.map(d => d.count), 5);
                            ...
                            return (
                                <div key={i} style={{
                                    ...
                                    <div className="chart-bar-container-tp" style={{ position: 'relative', width: '40px', flex: 1, display: 'flex', alignItems: 'flex-end', marginBottom: '12px', cursor: 'pointer' }}>
                                        <div
                                        ...
                                        >
                                            {item.count > 0 && (
                                                <div className="chart-tooltip-tp" style={{ position: 'absolute', top: '-24px', left: '50%', transform: 'translateX(-50%)', backgroundColor: '#0f172a', color: 'white', fontSize: '0.75rem', padding: '4px 8px', borderRadius: '6px', fontWeight: 700, whiteSpace: 'nowrap', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)', zIndex: 10 }}>
                                                    {item.count} tasks
                                                </div>
                                            )}
                                        </div>
                                    </div>
                                    <span style={{ fontSize: '0.875rem', color: '#64748b', fontWeight: 500 }}>{item.day}</span>
                                </div>
                            );
                        })}
                    </div>
