...
export const LoginPage = () => {
    ...
    const isSessionExpired = new URLSearchParams(location.search).get('session_expired') === 'true';
                ...
                <div className="bg-white border border-graphite/5 rounded-2xl p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-shadow duration-300">
                    ...
                    <form onSubmit={handleLogin} className="space-y-6">
                        ...
                        {isSessionExpired && !error && (
                            <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-xl flex items-center gap-3 text-amber-800 animate-in fade-in slide-in-from-top-4 duration-500">
                                <AlertCircle size={20} className="shrink-0" />
                                <div className="text-sm font-medium">
                                    your session is timed out please re login
                                </div>
                            </div>
                        )}
