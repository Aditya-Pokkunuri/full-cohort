...
export const ToastProvider = ({ children }) => {
    ...
    const addToast = useCallback((message, type = 'info', duration = 3000, component = null) => {
        const id = Date.now();
        setToasts((prev) => [...prev, { id, message, type, component }]);
        if (duration > 0) {
            setTimeout(() => {
    ...
    return (
        <ToastContext.Provider value={{ addToast }}>
            ...
            <div style={{
                ...
                {toasts.map((toast) => (
                    <div key={toast.id}>
                        {toast.type === 'custom' && toast.component ? (
