...
const DashboardHome = () => {
    ...
    const { userName, currentTeam, teamId, orgName } = useUser();
                ...
                if (profile?.team_id) {
                    ...
                    if (membersError) {
                        ...
                        const membersWithStatus = await Promise.all(
                            members.map(async (member) => {
                                ...
                                if (att && att.clock_in && !att.clock_out) {
                                    availability = 'Online';
                                } else if (leaveSet.has(member.id) && orgName !== 'Cohort') {
                        ...
                        setEmployeeStats({
                            ...
                            away: orgName === 'Cohort' ? 0 : membersWithStatus.filter(m => m.status === 'On Leave').length,
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px', paddingBottom: '32px' }}>
            ...
            <div>
                ...
                <p style={{ color: '#64748b', fontSize: '1rem' }}>
                    Wish you a good and productive day. {displayStats.active} team members active today. You have {filteredTimeline.length} events on {selectedDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}.
                </p>
            </div>
